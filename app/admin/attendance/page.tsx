    ...
    <div>
      <header className="mb-5">
        <div className="flex items-baseline gap-3 mb-1">
          ...
          <span className="text-xs uppercase tracking-wider text-cream-400 font-mono">
        ...
        {/* Pipeline counts strip — derived from kanban columns, always visible */}
        <div className="mt-4 flex items-center gap-1 flex-wrap text-[10px] uppercase tracking-wider font-mono">
          {KANBAN_ORDER.map((col, i) => (
            <span key={col} className="flex items-center gap-1">
              {i > 0 ? <span className="text-cream-500">→</span> : null}
              <span className="text-cream-400">{KANBAN_LABEL[col]}</span>
              <span className="text-cream-100">{counts.byCol.get(col) ?? 0}</span>
            </span>
          ))}
        </div>
      </header>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <input
          ...
          className="bg-brown-800 border border-brown-700 text-cream-50 text-xs px-2.5 py-1.5 focus:outline-none focus:border-orange-500 w-64"
        />
        <Select
