...
// Windbreaker event window: reviews from 2026-05-07 00:00 ET through
// 2026-05-15 23:59:59.999 ET inclusive. Anchor in Eastern Time so boundaries
// match the calendar day HC participants see, regardless of server TZ.
const WINDOW_START = new Date("2026-05-07T00:00:00-04:00")
const WINDOW_END = new Date("2026-05-16T00:00:00-04:00") // exclusive upper bound

const WINDBREAKER_THRESHOLD = 100
...
export async function GET() {
  ...
  const entries = reviewerIds
    .map((id) => {
      ...
      const tier: "windbreaker" | "none" = count >= WINDBREAKER_THRESHOLD ? "windbreaker" : "none"
      return {
        reviewer: {
          id,
  ...
  return NextResponse.json({
    ...
    windbreakerThreshold: WINDBREAKER_THRESHOLD,
    entries,
  })
}
